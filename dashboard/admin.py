...
class AdminActions:
    ...
    def soft_delete(self, run_id: str, target_id: str | None, token: str) -> dict:
        ...
        trash = run / "trash" / (target_id or run_id)
        trash.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(source), str(trash))
        with (run / "admin_events.jsonl").open("a", encoding="utf-8") as stream:
    ...
    def reset_dedupe(self, run_id: str, token: str) -> dict:
        ...
        marker.write_text(json.dumps({"time": datetime.now(timezone.utc).isoformat()}, ensure_ascii=False), encoding="utf-8")
